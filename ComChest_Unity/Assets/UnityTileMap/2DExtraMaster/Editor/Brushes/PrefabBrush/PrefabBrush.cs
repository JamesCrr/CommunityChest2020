...
namespace UnityEditor.Tilemaps
{
    ...
    public class PrefabBrush : GridBrush
    {
        ...
        /// <summary>
        /// Creates a selected Prefab from the array and positions it onto the grid position
        /// </summary>
        /// <param name="gridLayout">Grid used for layout.</param>
        /// <param name="brushTarget">Target of the paint operation. By default the currently selected GameObject.</param>
        /// <param name="position">The coordinates of the cell to paint data to.</param>
        /// <param name="arrayIndex">What prefab to use from the array</param>
        /// <returns></returns>
        public GameObject PlaceGO(GridLayout grid, GameObject brushTarget, Vector3Int position, int arrayIndex = 0)
        {
            if (position == prev_position)
                return null;
            if (arrayIndex < 0 || arrayIndex >= m_Prefabs.Length)
                return null;
            prev_position = position;
            if (brushTarget)
                prev_brushTarget = brushTarget;
            ...
            GameObject prefab = m_Prefabs[arrayIndex];
            GameObject instance = (GameObject)PrefabUtility.InstantiatePrefab(prefab);
            if (instance != null)
            {
                Erase(grid, brushTarget, position);
        ...
        /// <summary>
        /// Returns the GameObject parented under parent, at grid position
        /// </summary>
        /// <param name="grid">Grid used for layout.</param>
        /// <param name="parent">TileMap that holds all the Prefabs</param>
        /// <param name="position">The coordinates of the cell to erase data from.</param>
        /// <returns></returns>
        public GameObject GetGOInCell(GridLayout grid, Transform parent, Vector3Int position)
        {
            int childCount = parent.childCount;
            for (int i = 0; i < childCount; i++)
            {
